...
- [x] 5.1 CI vérifiée — run normal `35444713414` (`db428f8`) vert sur Linux Node 22/24, macOS et Windows Node 24, et Chromium Linux avec actions sous Node 24 ; artefact `npm-package-tested` téléchargé (SHA-256 `ee371f42ff2a33c760ee018567e514e3ea0ec7ba3804e61ee8ddc469696adc85`) ; PR brouillon [#1](https://github.com/OthmaneBlial/Regex-For-Humans/pull/1) volontairement cassée, run `35444252927` échoué dans les quatre jobs `npm run check`, puis fermée sans fusion
- [ ] 5.2 Package publié et atelier déployé — tarball local testé, workflows npm et Pages manuels depuis un tag stable préparés et contrôlés par `actionlint` ; build local chargé sous `/Regex-For-Humans/` avec recette, guide et assets en HTTP 200, sans erreur console ; npm et Pages publics encore absents
