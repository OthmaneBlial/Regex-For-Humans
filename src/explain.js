function repetitionText(repetition) {
  ...
  switch (repetition.kind) {
    case "zeroOrMore": return " Repeated zero or more times, greedily.";
    case "oneOrMore": return " Repeated one or more times, greedily.";
    case "optional": return " Optional: zero or one occurrence.";
    case "exact": return ` Repeated exactly ${repetition.min} times.`;
    case "range": return ` Repeated between ${repetition.min} and ${repetition.max} times, greedily.`;
    case "minimum": return ` Repeated at least ${repetition.min} times, greedily.`;
...
export function explainNode(node, flags) {
  ...
  switch (node.atomType) {
    ...
    case "shorthand":
      meaning = {
        "\\w": "One JavaScript word character: ASCII letter, digit or underscore. With i and u, a few Unicode case-folding equivalents also match.",
