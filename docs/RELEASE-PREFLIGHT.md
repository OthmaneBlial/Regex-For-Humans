...
1. Publish the **tested tarball** to npm with the intended account. Verify the registry's exact version, then install it from the public registry in a fresh directory and run the documented CLI and library examples. If the name has become unavailable, choose a new package name, update all docs and package metadata, and repeat the candidate checks before publication.
   The repository also contains a manual `.github/workflows/publish-npm.yml` workflow. It checks out an existing stable tag, refuses a `-dev` version or tag mismatch, requires the literal `PUBLISH` input, reruns the quality/package gates, and invokes npm trusted publishing with provenance. Configure the npm trusted publisher and the `npm-release` environment before using it. A completed workflow still requires the registry and clean-consumer checks below.
